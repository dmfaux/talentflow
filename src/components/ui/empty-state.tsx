...
const ICONS: Record<string, React.ReactNode> = {
  campaigns: (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#9fb5c4" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <div className="rounded-xl border border-border bg-surface px-5 py-14 text-center">
      ...
      {actionLabel && actionHref && (
        <Link
          href={actionHref}
          className="mt-4 inline-flex h-8 items-center rounded-lg bg-accent px-4 text-[0.75rem] font-medium text-ink transition-colors hover:bg-accent-light hover:text-white"
        >
          {actionLabel}
        </Link>
      )}

...
export default async function CampaignDetailPage({ params, searchParams }: Props) {
  ...
  // Candidate query with filters, sorting, pagination
  const limit = 20;
  ...
  const orderBy =
    sort === "score_asc"
      ? [asc(candidates.ai_score)]
      : sort === "date_desc"
        ? [desc(candidates.created_at)]
        : sort === "date_asc"
          ? [asc(candidates.created_at)]
  ...
  return (
    <div>
      ...
      <div className="mb-6 rounded-xl border border-border bg-surface px-6 py-5">
        <h3 className="mb-5 text-[0.65rem] font-semibold uppercase tracking-[0.14em] text-txt-muted">
          Pipeline
        </h3>
          ...
          {pipeline.map((stage) => {
            ...
            return (
              <div key={stage.label} className="flex items-center gap-4">
                <span className="w-28 shrink-0 text-[0.75rem] font-medium text-txt-secondary">
                  {stage.label}
                </span>
                <div className="relative flex-1 h-8 rounded-lg bg-cream overflow-hidden">
                  ...
                  />
                  <span className="absolute inset-y-0 left-3 flex items-center font-mono text-xs font-semibold text-charcoal">
                    {stage.value}
                  </span>
                </div>
              </div>
      ...
      {/* Candidate table */}
      <Suspense fallback={<div className="rounded-xl border border-border bg-surface p-10 text-center text-sm text-txt-muted">Loading candidates...</div>}>
        <CandidateTable
          campaignId={id}

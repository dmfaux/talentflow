...
// Shared with the dashboard + detail page so a status reads identically everywhere.
...
// Inline filter controls share one look until a FilterBar primitive is extracted
// (the same toolbar recurs on candidates / users / clients).
const filterControl =
  "h-9 rounded-lg border border-rule bg-surface text-[0.78rem] text-ink-soft outline-none transition-colors focus:border-cobalt focus:ring-1 focus:ring-cobalt/20";
...
function NewCampaignDropdown() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div role="menu" className="absolute right-0 top-full z-20 mt-1.5 w-52 overflow-hidden rounded-lg border border-rule bg-surface shadow-lg animate-[scaleIn_150ms_ease-out]" style={{ transformOrigin: "top right" }}>
          <Link
            href="/campaigns/new"
            role="menuitem"
            onClick={() => setOpen(false)}
            className="flex items-center gap-2.5 px-3.5 py-2.5 text-[0.8rem] text-ink transition-colors hover:bg-canvas"
          >
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <input
            ...
            className={`${filterControl} w-full pl-8 pr-3 placeholder:text-ink-muted`}
          />
          {search && (
            <button
              onClick={() => setSearch("")}
        ...
        <select
          ...
          className={`${filterControl} px-2.5 font-medium capitalize cursor-pointer`}
        >
          {STATUSES.map((s) => (
        ...
        <select
          ...
          className={`${filterControl} px-2.5 font-medium cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed`}
        >
      ...
      {loading ? (
        <div className="overflow-hidden rounded-xl border border-rule bg-surface">
          {Array.from({ length: 8 }).map((_, i) => (
            <div
              key={i}
              className="flex items-center gap-4 border-b border-rule px-5 py-3.5 last:border-b-0"
            >
        ...
        <div className="rounded-xl border border-rule bg-surface py-20 text-center">
          <p className="text-sm text-ink-soft">No campaigns match your filters</p>
          <button
            onClick={clearFilters}
                ...
                >
                  <td className="px-5 py-3">
                    <Link
                      ...
                      className="block text-sm font-medium text-ink group-hover:text-cobalt transition-colors"
                    >
                      {campaign.role_title}
                    </Link>
                    <p className="mt-0.5 font-mono text-[0.62rem] text-ink-muted">
                      {campaign.client_slug}/{campaign.slug}
                    </p>
                  </td>
                  <td className="px-5 py-3 text-sm text-ink-soft">
                    {campaign.client_name ?? <span className="text-ink-muted">&mdash;</span>}
                  </td>
                  <td className="px-5 py-3 text-sm text-ink-soft">
                    {campaign.department ?? <span className="text-ink-muted">&mdash;</span>}
                  </td>
                  <td className="px-5 py-3 text-sm text-ink-soft">
                    {campaign.location ?? <span className="text-ink-muted">&mdash;</span>}
                  </td>
                  <td className="px-5 py-3">
      ...
      >
        ...
        <span className="flex flex-col leading-none">
          <svg
            ...
            strokeWidth="1.5"
            strokeLinecap="round"
            className={active && currentDir === "asc" ? "text-cobalt" : "text-ink-muted/50"}
          >
          ...
          <svg
            ...
            strokeWidth="1.5"
            strokeLinecap="round"
            className={active && currentDir === "desc" ? "text-cobalt" : "text-ink-muted/50"}
          >

...
> **✅ DONE** — implemented 2026-06-19. Tenant routes (`GET /api/admin/themes` availability feed; `POST /api/admin/themes/test-send` + `?preview=1`; extended `PATCH /api/admin/clients/[id]` for `default_theme_id`; `theme_id` on campaign `POST`/`PATCH`), all guarded by a single shared `assertThemeAvailableForBrand` (`src/lib/theme.ts`) that collapses every unavailability — missing / inactive / sibling-bespoke / custom-on-Standard — to a `400`. Frontend (admin palette, `frontend-design`): an **Email Theme** picker in the wizard's Landing-Page step (inherit + theme cards, brand-default badge, live `<iframe>` email preview, self-only test-send) and a **Default Campaign Theme** selector in brand settings, sharing a `ThemeCard` component. **Decisions:** `default_theme_id` rides the existing `brand_admin` branding gate (RD-1); campaign create stores `theme_id: null` to inherit, never snapshotting the brand default (RD-2); test-send uses `sendTransactionalEmail` (never metered) with a light per-user throttle (RD-3). **Hardening:** `GET …?brand_id=` is org-scoped via `resolveOwnedResource` before the membership check, so an owner can't read a cross-org brand's bespoke. Verified: 6 unit (`assertThemeAvailableForBrand`) + 19 integration tests (availability scoping, cross-org/sibling/inactive rejection, campaign + brand-default writes, RBAC, unmetered test-send); `npm run build` clean. All acceptance criteria met.
